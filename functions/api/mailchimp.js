...
export async function onRequest(context) {
  ...
  const API_KEY = (env.MAILCHIMP_API_KEY || '').trim();
  if (!API_KEY) {
  ...
  const SERVER = API_KEY.split('-').pop().trim();
  ...
  try {
    ...
    const [account, lists, campaigns, automations] = await Promise.all([
      ...
      mcFetch('/campaigns?count=20&sort_field=send_time&sort_dir=DESC&fields=campaigns.id,campaigns.archive_url,campaigns.settings.title,campaigns.settings.subject_line,campaigns.emails_sent,campaigns.report_summary,campaigns.send_time,campaigns.status,campaigns.type'),
